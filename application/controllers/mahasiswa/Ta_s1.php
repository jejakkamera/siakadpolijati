...
class Ta_s1 extends CI_Controller {
		...
		if (($this->form_validation->run() == TRUE)   ) {
            ...
            else
            {
                ...
                if($ta_s1_mhs_sidang){
                    ...
                    $this->Master_model->update_query(['id_trx'=>$ta_s1_mhs_sidang->id_trx,'periode'=>$rule_date->periode,'nim'=>$this->session->userdata('username')], $data, 'ta_s1_mhs_sidang');
    ...
    public function pendaftar_yudisium_ta_s1($id_trx){
        ...
        $now = strtotime(date('Y-m-d'));
                ...
                //cek UKT
                $this->cek_ukt('mahasiswa/ta_s1/dashboard/'.$rule_date->periode);
    ...
    public function add_bimbingan_action($nim,$periode){
        ...
        if(($this->form_validation->run() == TRUE)){
            ...
            if(!empty($_FILES['berkas']['name'])){
                ...
                $data_masters=($this->Master_model->master_get(['nim' => $nim,'periode'=>$periode],'v_ta_s1_mhs_list'));
    ...
    public function add_bimbingan($nim,$periode){
        $this->load->model('form/Mhs_f', 'mahasiswa_form');

        $data_master=$this->mahasiswa_form->add_bimbingan_s1($nim,$periode);
    ...
    public function cek_ukt($link){
        ...
        if($data_json->status_code=='000'){
            ...
            foreach($data_json->data as $row){
                $sisa=((int) $row->total_tagihan)-((int) $row->total_bayar);
                if($sisa>0){
                    $text = $this->alert->warning('anda belum melakukan pembayaran UKT');
                    $this->session->set_flashdata('message', $text);
                    redirect($link);
                }
            }
            ...
            $text = $this->alert->warning('Error connection SIKU');
            $this->session->set_flashdata('message', $text);
            redirect($link);
        }
