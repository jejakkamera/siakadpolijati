...
class Wisuda extends CI_Controller {
    ...
    public function form_ijazah(){
        $wisuda=$this->Master_model->master_get(['nim'=>$this->session->userdata('username')],'v_mahasiswa');
        ...
        if($data_json){
            ...
            $this->header();
            $this->load->view('mahasiswa/form_ijazah',
                    [
                        'hutang'=>$hutang,
                        'wisuda'=>$wisuda,
                        'data_hutang'=>$data_json->data,
                    ]
            );
            $this->footer();

        }else{
            $text = $this->alert->danger('Sikku disconect');
                    $this->session->set_flashdata('message', $text);
                    redirect("welcome/dashboard");
        }
       
        // echo "Form pengambilan Ijazah dan transkript Nilai</br>";
        // echo $wisuda->nama.'</br>';
        // echo $wisuda->nim.'</br>';
        // foreach($data_json->data as $row){
        //     $sisa=((int) $row->total_tagihan)-((int) $row->total_bayar);
            
        //     echo "periode ".$row->periode." Bayar : ".$row->total_bayar." Tagihan : ".$row->total_tagihan." Sisa : ".$sisa.'</br>';
        // }
        // echo "Anda belum melunasi tagihan. silahkan hubungi bagian keuangan.";
        // print_r($data_json);
    }
