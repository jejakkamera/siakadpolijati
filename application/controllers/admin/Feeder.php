...
class Feeder extends CI_Controller {
    ...
    function f_GetJenisAktivitasMahasiswa(){
        $data_pt=$this->Master_model->master_get(['kode_pt'=>'043139'],'dikti_pt');
        $deskripsi = $this->feeder_lib->deskripsi($data_pt);
        $data=$deskripsi['data_pt'];
        $url=$deskripsi['url'];
        ...
        $data = ['act'=>'GetJenisAktivitasMahasiswa', 'token'=>$data->token];
        $data = json_encode($data);
        $data_feeder=$this->feeder_lib->send_ws($data,$url);
        //echo $data_feeder['error_code'];
        if($data_feeder['error_code']==0){ 
	...
	public function f_UpdateBiodataMahasiswa_list()
	{
    ...
    //    print_r($InsertPesertaKelasKuliah);die();
    ...
    public function f_GetListKelasKuliah(){
        $data_pt=$this->Master_model->master_get(['kode_pt'=>'043139'],'dikti_pt');
        $deskripsi = $this->feeder_lib->deskripsi($data_pt);
        $data=$deskripsi['data_pt'];
        $url=$deskripsi['url'];
        ...
        $data = ['act'=>'GetListKelasKuliah', 'filter'=>"id_semester='".$this->session->userdata('set_periode')['periode']."'", 'token'=>$data->token];
        $data = json_encode($data);
        $data_feeder=$this->feeder_lib->send_ws($data,$url);
        if($data_feeder['error_code']==0){ 
            foreach($data_feeder['data'] as $row){
                $this->Master_model->update_query(['id_matkul'=>$row->id_matkul,'periode'=>$this->session->userdata('set_periode')['periode'],'id_kelas_feeder is null'=>null], ['id_kelas_feeder'=>$row->id_kelas_kuliah], 'mata_kuliah_jadwal');
            }
            $text = $this->alert->success('please Cek Feeder');
            $this->session->set_flashdata('message', $text);
            redirect('admin/feeder/push_kelas_perkuliahan', 'refresh');
    ...
    public function f_GetNilaiTransferPendidikanMahasiswa(){
        if($this->session->userdata('set_periode')['periode']){
            ...
            // $data = ['act'=>'GetNilaiTransferPendidikanMahasiswa', 'filter'=>"", 'token'=>$data->token];
            $data = json_encode($data);
            $data_feeder=$this->feeder_lib->send_ws($data,$url);
            // print_r($data_feeder);die();
            if($data_feeder['error_code']==0){ 
    ...
    public function f_UpdateBiodataMahasiswa(){
        $data_pt=$this->Master_model->master_get(['kode_pt'=>'043139'],'dikti_pt');
        $deskripsi = $this->feeder_lib->deskripsi($data_pt);
        $data=$deskripsi['data_pt'];
        $url=$deskripsi['url'];
        $kode_pt=$data_pt->username;
        $this->load->model('Baak_model');
        $UpdateBiodataMahasiswa=$this->Baak_model->f_UpdateBiodataMahasiswa();
        $count=0;
        $count_push=0;
        $count_null=0;
        if($UpdateBiodataMahasiswa){
            $text = $this->alert->success('No Data');
            $this->session->set_flashdata('message', $text);
            redirect('admin/feeder/f_UpdateBiodataMahasiswa_list', 'refresh');
        }
        foreach($UpdateBiodataMahasiswa as $biodata){
            $data_bio=array(
                ...
                'nisn'=>$biodata->nisn,
                'npwp'=>$biodata->npwp,
                'id_negara'=>$biodata->id_negara,
                'kewarganegaraan'=>$biodata->kewarganegaraan,
                'dusun'=>$biodata->alamat,
                ...
                'nama_ibu'=>$biodata->nama_ibu_kandung,
                ...
                'id_pendidikan_wali'=>$biodata->id_jenjang_pendidikan_ibu,
            ...
            $payload = [
                ...
                print_r($data_feeder);die();
                if($data_feeder['error_code']==0){ 
                    $this->Master_model->update_query(['nim'=>$biodata->nim], ['update'=>'no','feeder_info'=>'UpdateBiodataMahasiswa : success update nilai'], 'mahasiswa_profile');
                    $count_push++;
                }else{
                    $this->Master_model->update_query(['nim'=>$biodata->nim], ['feeder_info'=>'UpdateBiodataMahasiswa : '.$data_feeder['error_desc']], 'mahasiswa_profile');
                    $count_null++;
                }
