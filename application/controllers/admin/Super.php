...
class Super extends CI_Controller {
	...
	public function update_user_action($user)
    {
        ...
        if ( ($this->form_validation->run() == TRUE)   ) {
				$data_masters=($this->Master_model->master_get(['SHA2(username,256)'=>$user],'user'));
				if($data_masters->password != $this->input->post('password',TRUE) ){
					$data = array(
						'password' => md5($this->input->post('password',TRUE)),
						'user_status' => $this->input->post('user_status',TRUE),
					);
					$this->Master_model->update_query(['SHA2(username,256)'=>$user], $data, 'user');
	...
	public function update_user($user)
    {
        ...
        $data_masters=get_object_vars($this->Master_model->master_get(['SHA2(username,256)'=>$user],'user'));
		if(!$data_masters){
