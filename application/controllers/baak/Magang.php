...
class Magang extends CI_Controller
{
    ...
    public function plot_dosen_action($id_trx)
    {
        ...
        if ($data_file) {
            ...
            if (($this->form_validation->run() == TRUE)) {
                ...
                if ($data_file->progress == 'daftar') {
                    // array_push($data,['progress'=>'bimbingan']);
                    $data['progres'] = 'bimbingan';
                }
    ...
    public function pengesahan($periode)
    {
        $this->header();
        $this->load->view(
            'baak/magang/dashboard',
        ...
        $this->load->model('form/Baak_f', 'baak_form');
        $data_master = $this->baak_form->pengesahan( $periode);
        $data_masters = get_object_vars($this->Master_model->master_get(['periode' => $periode], 'magang_list'));
    ...
    public function json_list_permohonan_magang()
    {
        $this->load->model('datatable/Baak_dt', 'Baak_dt');

        header('Content-Type: application/json');
        echo $this->Baak_dt->json_list_permohonan_magang();
    }
    ...
    public function list_permohonan()
    {
        ...
        $this->load->model('tabel/Baak_t', 'baak_tabel');
        $data_master = $this->baak_tabel->list_permohonan_magang();
    ...
    public function edit_permohonan_magang($id)
    {
        ...
        $data_master = $this->baak_form->edit_permohonan_magang($id);
        $this->header();
        $this->load->view(
            'master/master_form',
            [
    ...
    public function edit_permohonan_magang_action($id)
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($cek_) {
                ...
                $this->session->set_flashdata('message', $text);
                redirect("baak/magang/list_permohonan/");
            } else {
            ...
            redirect("baak/magang/edit_permohonan_magang/" . $id);
        }
    }
    ...
    {
        $this->load->model('tabel/Baak_t', 'baak_tabel');
        $data_master = $this->baak_tabel->list_mbkm();
	...
	public function download_surat_permohonan($id_periode){
		

		$pengesahan = $this->Master_model->master_get(['periode'=>$id_periode], 'magang_list');
			if(!$pengesahan->ttd){
				$text = $this->alert->danger('Tidak bisa unduh surat,Penegsah not found');
                $this->session->set_flashdata('message', $text);
                redirect("baak/dashboard/");
			}
		$cek_=(object) [
							'tujuan'=>'Tujuan surat',
							'persone'=>'Bapak / Ibu',
							'nama_penerima'=>'Tujuan Nama Penerima',
							'jabatan_penerima'=>'Tujuan Jabatan',
							'nama_perusahaan'=>'Tujuan Nama Instansi',
		...
		$this->load->library('Master_lib');
		 $this->load->view('mahasiswa/magang/surat_permohonan', [
                ...
                'tanggal' => $this->master_lib->format_indo(date("Y-m-d H:i:s")),
                'qr_code' => $dataUri,
                'uuid' => $cek_->uuid
            ]);
	}

	public function edit_pengesahan_action($id_periode)
    {
        $rule_date = $this->Master_model->master_get(['periode' => $id_periode], 'magang_list');
        if (!$rule_date) {
        ...
        if (($this->form_validation->run() == TRUE)) {
			$new_name =$rule_date->ttd;
			if (!empty($_FILES['berkas']['name'])) {
                $config['upload_path']          = './assets/berkas/pengesahan/';
                $config['allowed_types']        = 'png|PNG|Png';
                $new_name = time().'.png';
                ...
                if (!$this->upload->do_upload('berkas')) {
                    ...
                    $this->session->set_flashdata('message', $text);
                    return redirect("baak/magang/pengesahan/" . $id_periode);
                }

            }
				$data = array(
                    'ttd' => './assets/berkas/pengesahan/'.$new_name,
                    'jabatan' => $this->input->post('jabatan', TRUE),
                    'nama' => $this->input->post('nama', TRUE),
                    'kode' => $this->input->post('kode', TRUE),
				);
				
                $this->Master_model->insert_history('pengesahan_magang', 'magang_list', json_encode($data));
				$this->Master_model->update_query(['periode' => $id_periode], $data, 'magang_list');
				$text = $this->alert->success('Pengesahan was update');
                $this->session->set_flashdata('message', $text);
                redirect("baak/magang/pengesahan/" . $rule_date->periode);

        } else {
            $this->pengesahan($id_periode);
        }
    }
}
