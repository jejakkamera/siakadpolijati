...
class Frs extends CI_Controller {
    ...
    public function jadwal_dosen_all_aktif()
	{
		if($this->session->userdata('set_periode')['periode']){
                $this->jadwal_dosen_all($this->session->userdata('set_periode')['periode']);
        }else{
                $text = $this->alert->danger('Select Periode');
                $this->session->set_flashdata('message', $text);
                redirect('baak/pembelajaran/set_periode/', 'refresh');
    ...
    public function rps($id){
        ...
        $this->header();
        $this->load->view('dosen/perangkat_pembelajaran',
            [
                'jadwal'=>$v_jadwal,
                'id_jadwal'=>$id,
                'pertemuan'=>$pertemuan,
    ...
    public function putusan_rps($id_trx_jadwal)
	{
        $this->Master_model->update_query(
            [
                'id_trx_jadwal'=>$id_trx_jadwal
            ], 
            [
                'status'=>$this->input->post('putusan',TRUE),
                'ket_rps'=>$this->input->post('keterangan',TRUE),
            ], 
            'mata_kuliah_jadwal'
        );
            log_message('info', 'Update - data to input_nilai_set, data :');
            ...
            $text = $this->alert->success('Data successfully update');
			$this->session->set_flashdata('message', $text);
			redirect(site_url('baak/frs/rps/'.$id_trx_jadwal));
    }

    public function rps_proses($periode)
	{
        $this->load->model('tabel/Baak_t', 'baak_tabel');
        // $data_master=$this->Baak_model->kurikulum_saji($id_kurikulum,$periode);
        // print_r($data_master);
        $data_master=$this->baak_tabel->rps_proses($periode);
