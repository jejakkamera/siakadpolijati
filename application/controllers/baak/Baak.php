...
class Baak extends CI_Controller {
	...
	public function perkuliahan_periode()
	{
		$this->load->model('tabel/Baak_t', 'baak_tabel');
        $data_master=$this->baak_tabel->perkuliahan_periode();
	...
	public function unhide_periode()
	{
		$this->load->model('tabel/Baak_t', 'baak_tabel');
        $data_master=$this->baak_tabel->unhide_periode();
	...
	public function perkuliahan_periode_unhide($id)
	{
		$where_=array(
			'id_periode'=>$id
		);
		$cek_=$this->Master_model->master_get($where_,'dikti_semester_hide');
		if($cek_){

			$where_array=array(
				'id_periode'=>$id
			);
			$this->Master_model->delete_query($where_array, 'dikti_semester_hide');

			$data_history=json_encode($cek_);
			log_message('info', 'Delete - data unhide, data :'.$cek_);

			$text = $this->alert->success('Data successfully Unhide');
				$this->session->set_flashdata('message', $text);
				redirect(site_url('baak/baak/unhide_periode'));
		}else{

			$text = $this->alert->danger('Periode not exists');
				$this->session->set_flashdata('message', $text);
				redirect(site_url('baak/baak/unhide_periode'));
		}
	}

	public function perkuliahan_periode_hide($id)
	{
		$where_=array(
			'id_periode'=>$id
		);
		$cek_=$this->Master_model->master_get($where_,'dikti_semester_hide');
		if($cek_){
			$text = $this->alert->danger('Periode already exists');
				$this->session->set_flashdata('message', $text);
				redirect(site_url('baak/baak/perkuliahan_periode'));
		}else{
			$data=['id_periode'=>$id,'creat_at'=>$this->Master_model->user_cek_ident()];
			$this->Master_model->insert_query('dikti_semester_hide',$data);
			$text = $this->alert->success('Data successfully Add');
				$this->session->set_flashdata('message', $text);
				redirect(site_url('baak/baak/perkuliahan_periode'));
		}
    ...
    public function edit_aktif_action(){
        ...
        if (($this->form_validation->run() == TRUE)) {
			$data_masters=$this->Master_model->master_get(['id' => 1],'pengesahan');
        
			$new_name =$data_masters->file;
			if (!empty($_FILES['file']['name'])) {
				...
				$data = array(
                    'file' => './assets/berkas/pengesahan/'.$new_name,
                    'jabatan' => $this->input->post('jabatan', TRUE),
                    'nama' => $this->input->post('nama', TRUE),
                    'kode' => $this->input->post('kode', TRUE),
				);
				
                $this->Master_model->insert_history('pengesahan_aktif', 'magang_list', json_encode($data));
				$this->Master_model->update_query(['id' => 1], $data, 'pengesahan');
				$text = $this->alert->success('Pengesahan was update');
                $this->session->set_flashdata('message', $text);
                redirect("baak/baak/pengesahan/");
    ...
    public function pengesahan()
	{
        ...
        $data_masters=get_object_vars($this->Master_model->master_get(['id' => 1],'pengesahan'));
        $data_master=$this->baak_form->pengesahan_aktif();
		$this->header();
        $this->load->view('master/master_form',
            [
                'data_detail'=>$data_master['form_detail'],
