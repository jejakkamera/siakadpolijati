...
class Pembelajaran extends CI_Controller {
    ...
    public function suspend_mahasiswa(){
            $this->load->model('tabel/Baak_t', 'baak_tabel');
            $data_master=$this->baak_tabel->suspend_mahasiswa();
    ...
    public function add_suspend_mahasiswa_select(){
        $this->load->model('form/Baak_f', 'baak_form');
        $data_master=$this->baak_form->set_suspend_mahasiswa_select();
    ...
    public function add_suspend_mahasiswa_action()
	{
        $duallistbox_demo1=$this->input->post('duallistbox_demo1',TRUE);
        $kode_prodi=$this->input->post('kode_prodi',TRUE);
        $id_periode=$this->input->post('id_periode',TRUE);
		$this->Baak_model->add_suspend_mahasiswa_action($kode_prodi,$id_periode,$duallistbox_demo1);
		$text = $this->alert->success('Data Successfully Suspend');
        $this->session->set_flashdata('message', $text);
        redirect(site_url('baak/pembelajaran/suspend_mahasiswa/'));
		// print_r($duallistbox_demo1);
    }

    

    public function suspend_mahasiswa_cancel_action($nim)
	{   
        $data=array(
            'suspend'=>'no',
        );
        $this->Master_model->update_query(['nim'=>$nim], $data, 'mahasiswa');
        $text = $this->alert->success('Data Successfully Cancel Suspend');
        $this->session->set_flashdata('message', $text);
        redirect(site_url('baak/pembelajaran/suspend_mahasiswa/'));
    }
	...
	{
		$this->load->model('form/Baak_f', 'baak_form');
        $data_master=$this->baak_form->cetak_kartu_ujian();
	...
	function cetak_kartu_ujian_action()
    {
		$id_periode=$this->input->post('id_periode',TRUE);
        $kode_prodi=$this->input->post('kode_prodi',TRUE);
        $id_periode_masuk=$this->input->post('id_periode_masuk',TRUE);
		$type = $this->input->post('type', TRUE);
		$list_of_nim=$this->Baak_model->cetak_kartu_ujian_action($id_periode,$kode_prodi,$id_periode_masuk);    
        // print_r($list_of_nim);die();
		// $data4 = $this->load->library('mypdf');
			if ($list_of_nim) {

				// Generate konten untuk halaman tertentu
				$data4 = $this->load->library('mypdf');
            	$this->mypdf->generates(
					'mahasiswa/cetak_kartu_ujians',
					[
						'periode' => $id_periode,
						'list_of_nim' => $list_of_nim,
						'type' => $type
					]
				);
    ...
    public function add_suspend_mahasiswa_list()
	{
        ...
        if(($this->form_validation->run() == TRUE)){
            ...
            // $id_periode=$this->input->post('id_periode',TRUE);
				...
				$this->load->model('tabel/Baak_t', 'baak_tabel');
				$data_master=$this->Baak_model->add_suspend_mahasiswa_list($data_where);    
            
            $this->header();
            $this->load->view('mahasiswa/add_suspend_mahasiswa_list',[
                'kode_prodi'=>$kode_prodi,
                'id_periode'=>$id_periode,
                'data_master'=>$data_master,
                'action'=>base_url('baak/pembelajaran/add_suspend_mahasiswa_action/'),
            ]);
            $this->footer();
        }else{
            $text = $this->alert->danger('Error data select');
            $this->session->set_flashdata('message', $text);
            redirect('baak/pembelajaran/add_suspend_mahasiswa_select/', 'refresh');
        }
	}
