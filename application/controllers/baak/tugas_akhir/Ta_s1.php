...
class Ta_s1 extends CI_Controller {
    ...
    public function tolak_yudisium($id_trx,$periode){
        $this->header();
        $this->load->view('baak/tugas_akhir_s1/dashboard',
            [
                'periode'=>$periode,
        ...
        $data_file = $this->Master_model->master_get(['id_trx'=>$id_trx], 'v_yudisium_mhs');

        $this->load->model('form/Baak_f', 'baak_form');
        $data_master=$this->baak_form->tolak_yudisium($id_trx,$data_file);
    ...
    public function tolak_yudisium_action($id_trx){
        $data_file = $this->Master_model->master_get(['id_trx'=>$id_trx], 'v_yudisium_mhs');
        if($data_file){
            $this->form_validation->set_rules('ket', 'ket', 'trim|required|xss_clean');
            $this->form_validation->set_error_delimiters('<span class="text-danger">', '</span>');
            if ( ($this->form_validation->run() == TRUE)   ) {
                ...
                $text = $this->alert->success('data update');
                $this->session->set_flashdata('message', $text);
                redirect("baak/tugas_akhir/ta_s1/yudisium_daftar/".$data_file->id_yudisium."/".$data_file->periode);
            }else{
                $this->tolak_yudisium($id_trx,$data_file->periode);
            }
        }else{
            $text = $this->alert->danger('Record not found');
				$this->session->set_flashdata('message', $text);
