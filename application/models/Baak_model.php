...
{ //baku
    ...
    function cetak_kartu_ujian_action($id_periode,$kode_prodi,$id_periode_masuk)
    {
		$this->db->select('mahasiswa.nim');
        $this->db->from('mahasiswa');
		$this->db->join('frs_mhs_mk', 'frs_mhs_mk.id_registrasi_mahasiswa=mahasiswa.id_registrasi_mahasiswa');
		$this->db->where('frs_mhs_mk.periode', $id_periode);
		$this->db->where('mahasiswa.kode_prodi', $kode_prodi);
		$this->db->where('mahasiswa.id_periode_masuk', $id_periode_masuk);
    ...
    function InsertPesertaKelasKuliah()
    {
        ...
        // $this->db->where('frs_mhs_mk.id_registrasi_mahasiswa','02ecc4bd-2051-431a-9df1-a25361938244');
    ...
    {
        $this->db->select(' id_kelas_feeder, sks_mata_kuliah,id_feeder_dosen,mata_kuliah_saji.id_matkul,id_trx_jadwal,id_registrasi_dosen');
        $this->db->from('v_jadwal');
        $this->db->join('dikti_prodi', 'dikti_prodi.kode_program_studi=v_jadwal.kode_prodi');
        $this->db->join('mata_kuliah_saji', 'mata_kuliah_saji.id_matkul=v_jadwal.id_matkul and mata_kuliah_saji.periode=v_jadwal.periode ');
        $this->db->join('pegawai', 'pegawai.id=mata_kuliah_saji.id_dosen_pengampu');
        $this->db->join('dikti_penugasan_dosen', 'dikti_penugasan_dosen.id_dosen=pegawai.id_dosen and dikti_penugasan_dosen.id_tahun_ajaran = v_jadwal.tahun');
    ...
    function add_suspend_mahasiswa_list($data_where)
    {
        $this->db->select('nim, nama, id_mahasiswa, id_registrasi_mahasiswa, id_periode_masuk, kode_prodi, nama_program_studi, nama_jenjang_pendidikan, status_mahasiswa, id_kurikulum, nama_kurikulum, info, id_kelas, nama_kelas, angkatan, suspend');
        $this->db->from('v_mahasiswa');
    ...
    function add_suspend_mahasiswa_action($kode_prodi, $id_periode, $duallistbox_demo1)
    {

        $this->db->query("UPDATE mahasiswa SET suspend='no' WHERE id_periode_masuk='" . $id_periode . "' and kode_prodi='" . $kode_prodi . "' ");
        if ($duallistbox_demo1) {
            $this->db->where_in('nim', $duallistbox_demo1);
            $this->db->update('mahasiswa', ['suspend' => 'yes']);
        }
    ...
    function v_frs_index($periode)
    {
        ...
        $this->db->where('frs_mhs_mk.transfer', 'tidak');
        $this->db->where('frs_mhs_mk.feeder_update_nilai', 'belum');
    ...
    function json_tahun_ajaran_by_tahun($mk, $tahun)
    {
        $this->db->select('id_tahun_ajaran as kode,id_tahun_ajaran as nama');
        $this->db->from('dikti_semester');
        $this->db->where('id_tahun_ajaran >=', $tahun);
        $this->db->where('id_tahun_ajaran <=', $tahun + 8);
        $this->db->like('id_tahun_ajaran', $mk);
        $this->db->group_by('id_tahun_ajaran');
    ...
    function f_UpdateBiodataMahasiswa()
    {
        $this->db->select('*');
        $this->db->from('mahasiswa_profile');
        $this->db->join('mahasiswa', 'mahasiswa.nim=mahasiswa_profile.nim');
        $this->db->where('update', 'yes');
        $query = $getData = $this->db->get();
    ...
    // funtion
    function get_list_mahasiswa_aktif_last_id($periode)
    {

        $sql = "SELECT nomer_aktif AS 'last_number'
                FROM list_mahasiswa_aktif 
                where year(`created_at`) = " . date("Y") . "
                ORDER BY nomer_aktif DESC
                LIMIT 1";
        $query = $this->db->query($sql, [$periode]);
        if ($query->num_rows() > 0) {
    ...
    function get_list_mahasiswa_cuti_last_id($periode)
    {
        $sql = "SELECT nomer_aktif AS 'last_number'
    ...
    // funtion
    function get_list_permohonan_magang_last_id($periode)
    {
        $sql = "SELECT nomer_aktif AS 'last_number'
                FROM permohonan_magang 
                WHERE periode = ?
                ORDER BY nomer_aktif DESC
                LIMIT 1";
        $query = $this->db->query($sql, [$periode]);
    ...
    function get_list_surat_kelulusan_last_id()
    {
        $sql = "SELECT nomer_aktif AS 'last_number'
                FROM list_surat_kelulusan 
                where year(`created_at`) = " . date("Y") . "
                ORDER BY nomer_aktif DESC
                LIMIT 1";
