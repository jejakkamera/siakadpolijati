...
class Dosen_t extends CI_Model{//baku
    ...
    function presensi_dosen($id_trx,$cek_)
    {
        $data_detail=array(
            ...
            'tabel_detail'=>'Presensi Kehadiran Dosen </br> Dosen : '.$cek_->nama_id_dosen.'</br> Matakuliah : '.$cek_->kode_mata_kuliah.'-'.$cek_->nama_mata_kuliah.' - '.$cek_->nama_kelas.'</br> Program Studi : '.$cek_->nama_program_studi,
    ...
    function perangkat_perkuliahan_list($periode)
    {
            $data_detail=array(
                'link_json'=>base_url('dosen/data_json/json_perangkat_perkuliahan/'.$periode->id_semester),
                'header'=>'Jadwal periode : '.$periode->nama_semester." (".$periode->id_semester.")",
                'tabel_detail'=>'Jadwal periode : '.$periode->nama_semester." (".$periode->id_semester.")",
            );
    ...
    function copy_draf($periode,$id_trx)
    {
            $data_detail=array(
                'link_json'=>base_url('dosen/data_json/json_copy_draf/'.$periode->id_semester.'/'.$id_trx),
                'header'=>'Jadwal periode : '.$periode->nama_semester." (".$periode->id_semester.")",
                'tabel_detail'=>'Jadwal periode : '.$periode->nama_semester." (".$periode->id_semester.")",
            );
    ...
    function presensi_mahasiswa_pertemuan_report($cek_)
    {
            $data_detail=array(
                ...
                'header'=>'Presensi Perkuliahan Mahasiswa, MK : '.$cek_->nama_mata_kuliah,
                'tabel_detail'=>'Presensi Kehadiran Mahasiswa </br> Dosen : '.$cek_->nama_id_dosen.'</br> Matakuliah : '.$cek_->kode_mata_kuliah.'-'.$cek_->nama_mata_kuliah.' - '.$cek_->nama_kelas.'</br> Program Studi : '.$cek_->nama_program_studi
            );

            if($this->session->userdata('role')!='mhs'){
                $data_detail['button_name']='Tarik Nilai';
                $data_detail['button_action_link']=base_url('dosen/perkuliahan/input_nilai_presensi/'.$cek_->id_trx_jadwal);
                $data_detail['button_icon']='Tarik fa fa-random';
            }
    ...
    function perangkat_perkuliahan_periode()
    {
            $data_detail=array(
                'link_json'=>base_url('dosen/data_json/json_perangkat_perkuliahan_periode/'),
                'header'=>'List Tahun Akademik ',
                'tabel_detail'=>'List Tahun Akademik',

                'button_name'=>'Periode Berjalan',
                'button_action_link'=>base_url('dosen/perkuliahan/perangkat_perkuliahan_periode_now'),
                'button_icon'=>'fa fa-calendar',
