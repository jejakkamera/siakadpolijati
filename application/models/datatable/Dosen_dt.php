...
class Dosen_dt extends CI_Model {
    ...
    function json_jadwal_dosen($periode)
	{
        ...
        $this->datatables->where_in('v_jadwal.status_rps',['proses','diterima']);
    ...
    function json_copy_draf($periode,$id_trx)
	{
        $this->datatables->select('id_trx_jadwal, id_matkul, periode, v_jadwal.id_dosen, id_kelas, ruang, waktu, nama_waktu, waktu_jam, hari, last_edit, kode_mata_kuliah, nama_mata_kuliah, kode_prodi, nama_id_dosen, id_status_aktif, email, status_pegawai, nama_kelas, prodi_kelas, angkatan,nama_program_studi,status_rps');
        $this->datatables->from('v_jadwal');
        // $this->datatables->join('dikti_prodi','v_jadwal.kode_prodi=dikti_prodi.kode_program_studi');
        $this->datatables->where('v_jadwal.id_trx_jadwal !=',$id_trx);
        $this->datatables->where('v_jadwal.periode',$periode);
        $this->datatables->where('v_jadwal.email',$this->session->userdata('username'));
        $this->datatables->add_column('action',
		anchor(
			site_url('dosen/perkuliahan/copy_draf_action/$1/'.$id_trx),'<i class="fa fa-copy"></i>','data-toggle="tooltip" data-html="true" title="Copy RPS" class="btn btn-success"')
		, 'id_trx_jadwal');
        return $this->datatables->generate();
    }

    function json_perangkat_perkuliahan($periode)
	{
        $this->datatables->select('id_trx_jadwal, id_matkul, periode, v_jadwal.id_dosen, id_kelas, ruang, waktu, nama_waktu, waktu_jam, hari, last_edit, kode_mata_kuliah, nama_mata_kuliah, kode_prodi, nama_id_dosen, id_status_aktif, email, status_pegawai, nama_kelas, prodi_kelas, angkatan,nama_program_studi,status_rps');
        $this->datatables->from('v_jadwal');
        // $this->datatables->join('dikti_prodi','v_jadwal.kode_prodi=dikti_prodi.kode_program_studi');
        ...
        $this->datatables->add_column('action',
		anchor(
			site_url('dosen/perkuliahan/perangkat_pembelajaran/$1'),'<i class="fa fa-pencil"></i>','data-toggle="tooltip" data-html="true" title="Upload RPS" class="btn btn-success"')
		, 'id_trx_jadwal');
        return $this->datatables->generate();
    }
    ...
    function json_frs_mahasiswa($periode)
	{
        ...
        $this->datatables->add_column('action',
		anchor(
			site_url('mahasiswa/profile/detail/$1'),'<i class="fa fa-user"></i>','data-toggle="tooltip" target="_blank" data-html="true" title="Profile" class="btn btn-warning"')." ".
		anchor(
			site_url('dosen/mahasiswa/transkrip_akademik/$1'),'<i class="fa fa-book"></i>','data-toggle="tooltip" target="_blank" data-html="true" title="Aktivitas Akademik" class="btn btn-success"')." ".
		anchor(
			site_url('dosen/mahasiswa/frs_mhs/$1/$2'),'<i class="fa fa-check-square"></i>','data-toggle="tooltip" target="_blank"  data-html="true" title="FRS" class="btn btn-info"')
		, 'nim,periode_a');
        return $this->datatables->generate();
    }
    ...
    function presensi_mahasiswa_pertemuan_report($cek_)
	{
        ...
        if(presensi_status_15="hadir",1,0)+
        if(presensi_status_16="hadir",1,0)+
        ...
        $this->datatables->where('v_frs.transfer','tidak');
        if($this->session->userdata('role')=='mhs'){
            $this->datatables->where('v_frs.nim',$this->session->userdata('username'));
        }
    ...
    function json_perangkat_perkuliahan_periode()
	{
        //dikti_semester
        $this->datatables->select('id_tahun_ajaran,id_semester,nama_semester',false);
        $this->datatables->from('dikti_semester');
        $this->datatables->join('mata_kuliah_jadwal','mata_kuliah_jadwal.periode=dikti_semester.id_semester');
        $this->datatables->join('pegawai','pegawai.id=mata_kuliah_jadwal.id_dosen');
        $this->datatables->where('pegawai.email',$this->session->userdata('username'));
        $this->datatables->group_by('dikti_semester.id_semester');
        $this->datatables->add_column('action',
		anchor(
			site_url('dosen/perkuliahan/perangkat_perkuliahan_list/$1'),'<i class="fa fa-info"></i>','data-toggle="tooltip" data-html="true" title="Detail" class="btn btn-success"')
		, 'id_semester');
        return $this->datatables->generate();
